    ...
    createOrUpdate (config) {
      return this.fetchAllServices()
        .then((existingServices) => {
          ...
          const result = diff(existingServices, allServices, (a, b) => { return a.title === b.title })
          const created = result.created
          const updated = result.updated

          var serviceActions = librato.createServices(created).concat(librato.updateServices(updated))
          return Promise.all(serviceActions)
        })
        .then(() => this.fetchAllAlerts())
        .then((existingAlerts) => {
